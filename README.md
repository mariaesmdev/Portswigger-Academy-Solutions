# Portswigger-Academy-Solutions
Resolução dos laboratórios da PortSwigger Web Security Academy. Um registro prático do meu desenvolvimento em Pentest e análise de vulnerabilidades.

# 💻 Portswigger Web Security Academy Labs 

![Status](https://img.shields.io/badge/Status-Em%20Evolu%C3%A7%C3%A3o-blue) 
![Foco](https://img.shields.io/badge/Foco-Red%20Team%20%26%20Pentest-red)

## 📖 Sobre este Repositório

Este repositório serve como meu *logbook* e base de conhecimento para a resolução dos desafios de cibersegurança da **PortSwigger Web Security Academy**.

Aqui, registro, scripts customizados (principalmente em Python) e metodologias utilizadas para explorar as vulnerabilidades em nível **Apprentice**, **Practitioner** e **Expert**.

O objetivo é aprimorar meu conhecimento prático em **Pentest** e fortalecer minhas habilidades em segurança ofensiva.


LABS DE: [XSS](./XSS)
